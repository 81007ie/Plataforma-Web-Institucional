// perfil.js (COMPLETO ACTUALIZADO CON BLOQUEO/DESBLOQUEO)
...
onAuthStateChanged(auth, async (u) => {
  ...
  try {
    ...
    if (!snap.exists() || snap.data().activo === false) {
      await signOut(auth);
      Swal.fire("Cuenta bloqueada", "Tu cuenta ha sido desactivada.", "error");
      return;
    }
...
function renderPagina(page) {
  ...
  if (pageItems.length === 0) {
    ...
    for (const u of pageItems) {
      ...
      if (usuarioActual.rol === "Administrativo") {
        if (u.activo === false) {
          botonesHTML = `<div class="btn-group">
            <button class="edit" data-id="${u.id}">Editar</button>
            <button class="unlock" data-id="${u.id}">Desbloquear</button>
          </div>`;
        } else {
          botonesHTML = `<div class="btn-group">
            <button class="edit" data-id="${u.id}">Editar</button>
            <button class="block" data-id="${u.id}">Bloquear</button>
          </div>`;
        }
      } else {
        botonesHTML = `<div class="btn-group">
  ...
  // Eventos
  tablaBody.querySelectorAll(".edit").forEach(btn => btn.onclick = e => {
    const id = e.currentTarget.dataset.id;
    const u = usuariosTodos.find(x => x.id === id) || {};
    openEditModal(id, u);
  });

  tablaBody.querySelectorAll(".block").forEach(btn => btn.onclick = async e => {
    const id = e.currentTarget.dataset.id;
    const u = usuariosTodos.find(x => x.id === id);
    const res = await Swal.fire({
      title: `Bloquear ${u?.nombre || "usuario"}?`,
    ...
    try {
      ...
      usuariosTodos = usuariosTodos.map(x => x.id === id ? { ...x, activo: false } : x);
      usuariosFiltrados = usuariosFiltrados.map(x => x.id === id ? { ...x, activo: false } : x);
      renderPagina(paginaActual);
  ...
  tablaBody.querySelectorAll(".unlock").forEach(btn => btn.onclick = async e => {
    const id = e.currentTarget.dataset.id;
    const u = usuariosTodos.find(x => x.id === id);
    const res = await Swal.fire({
      title: `Desbloquear ${u?.nombre || "usuario"}?`,
    ...
    try {
      ...
      usuariosTodos = usuariosTodos.map(x => x.id === id ? { ...x, activo: true } : x);
      usuariosFiltrados = usuariosFiltrados.map(x => x.id === id ? { ...x, activo: true } : x);
      renderPagina(paginaActual);
  ...
  tablaBody.querySelectorAll(".ver").forEach(btn => btn.onclick = e => {
    const id = e.currentTarget.dataset.id;
    const u = usuariosTodos.find(x => x.id === id) || {};
    Swal.fire({
...
function renderChips() {
  ...
  for (const p of pages) {
    if (p === "left-ellipsis" || p === "right-ellipsis") {
      ...
      btn.textContent = p;
      btn.onclick = () => { if (p !== paginaActual) renderPagina(p); };
...
if (buscador) {
  buscador.addEventListener("input", () => {
    ...
    usuariosFiltrados = !term ? [...usuariosTodos] :
...
// ---------- PREV / NEXT ----------
if (btnPrev) btnPrev.onclick = () => { if (paginaActual > 1) renderPagina(paginaActual - 1); };
if (btnNext) btnNext.onclick = () => { if (paginaActual < totalPaginas) renderPagina(paginaActual + 1); };
...
async function cargarComunicados() {
  try {
    ...
    if (snap.empty) { listaComunicadosCont.innerHTML = "<p>No hay comunicados.</p>"; return; }
